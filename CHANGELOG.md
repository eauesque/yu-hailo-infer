...
- **`/v1/infer/wd` の 500 応答が原因を一切載せていなかった問題を修正。** 本文は
  `{"error":"inference failed"}` の固定文字列で、実際の `InferError` は
  サイドカー自身のログにしか出ていなかった。呼び出し側(yu-server)は本文を
  そのまま `BackendError` に包んでバッチジョブのラベルに載せるため、運用者が
  見る場所には理由が届かない。既存の `unsupported profile` と同じ流儀で
  `detail` フィールドを足し、`e.to_string()` を載せるようにした。
  - **区別できねば応答が正反対になる。** 画像1枚の破損(`InferError::Image`)は
    そのファイルを飛ばせば済むが、ONNX セッションの死(`InferError::Ort`)は
    後続の全ファイルが同じく落ちる。同じ 500・同じ文言では、yu-server は
    「BackendError が20回連続したら中断」という当て推量で凌ぐしかない。
  - 同じ欠陥を持つ他の2箇所(profile fingerprint 失敗・`spawn_blocking` panic)
    にも `detail` を足した。1箇所直して同族を残せば次も同じ所で詰まる。
  - `error` フィールドの値は従来のまま据え置いた。本文を文字列照合している
    呼び出し側があっても壊れない。
